 ...
 * @param result
 */
class RootedDeviceUseCase(
    context: Context, analysisItem: ModuleEntity,
    result: AnalysisResultEntity,
) : BaseAnalysisUseCase(context, analysisItem, result) {

    override fun analyse() {
        if (isDeviceRooted()) {
            result.device.isRooted = 1
            itemResult.notOk = result.device.isRooted == 1
        }
    }
