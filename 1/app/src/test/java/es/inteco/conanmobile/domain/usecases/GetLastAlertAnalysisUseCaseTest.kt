...
class GetLastAlertAnalysisUseCaseTest {
    ...
    @Test
    fun getLastAlertAnalysis_should_get_last_alert_analysis_from_preferences() {
        Mockito.`when`(preferencesRepositoryFactory.create(Strategy.PREFERENCES))
            .thenReturn(preferencesRepository)
        Mockito.`when`(preferencesRepository.getLastAlertAnalysis())
            .thenReturn(
                Mockito.anyLong()
            )
        ...
        verify(preferencesRepository).getLastAlertAnalysis()
    }

    private fun createUseCase(): GetLastAlertAnalysisUseCase =
