...
class GetNextAnalysisDateTimeUseCaseTest {
    ...
    @Test
    fun getNextAvailableAnalysisDateTime_should_get_next_available_analysis_date_time_from_preferences() {
        Mockito.`when`(preferencesRepositoryFactory.create(Strategy.PREFERENCES))
            .thenReturn(preferencesRepository)
        Mockito.`when`(preferencesRepository.getNextAvailableAnalysisDateTime())
            .thenReturn(
                Mockito.anyLong()
            )
        ...
        verify(preferencesRepository).getNextAvailableAnalysisDateTime()
    }

    private fun createUseCase(): GetNextAnalysisDateTimeUseCase =
